...
def data_processing(_newData):
	...
	if newType == "0500":
		...
		if collected_data_index <= kalman_sample_size:
			pass
		elif collected_data_index <= sample_size:
			global x0
			global K
			global Dxx
			global Ixx
		...
		elif collected_data_index == (sample_size + 1):
			...
			if collected_data_index == max_data_size:
				# save data
				text_file_x = open("data_Total.txt","wt")
				w_text_x = text_file_x.write(collected_data_x)
				text_file_x.close()

				text_file_u = open("data_Steering.txt","wt")
				w_text_u = text_file_u.write(collected_data_u)
				text_file_u.close()

				text_file_pos = open("data_pos.txt","wt")
...
def model(x, k, K, ifLEarned):
	if ifLEarned == 0:
		ExpNoise = 0.1 * np.sin(10*k)
		u = -np.matmul(K,x)+ExpNoise ## use as steering input
	else:
		u = -np.matmul(K,x)
...
x0 = np.array([[1],[0],[1],[0]])
K = np.array([[0.0146,   -0.0750,    2.0831,    0.8227]]) #Opt
#K = np.array([[0.0046,   -0.0450,    1.9931,    0.0026]]) #init
#K = np.array([[0.0246,   -0.0450,    1.0931,    0.0026]])
#K = np.array([[0.0146,   -0.0750,    0.7831,    0.3227]])
